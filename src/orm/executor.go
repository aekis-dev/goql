...
// processIfStatement converts if statement to ConditionalUpdate
func (e *DebugExecutor) processIfStatement(ifStmt *ast.IfStmt, tableName string, entityType reflect.Type) (*ConditionalUpdate, error) {
	...
	whereClause := e.exprToSQL(ifStmt.Cond)
	...
	for _, stmt := range ifStmt.Body.List {
		if assignStmt, ok := stmt.(*ast.AssignStmt); ok {
			// Process assignment: c.Field = value
	...
	if len(setClauses) == 0 {
		return nil, nil // No assignments found
	}
...
func (e *DebugExecutor) astToSQL(fn *ast.FuncLit) string {
	...
	return e.exprToSQL(retStmt.Results[0])
}

// exprToSQL converts an AST expression to a SQL fragment
func (e *DebugExecutor) exprToSQL(expr ast.Expr) string {
	switch v := expr.(type) {
	case *ast.BinaryExpr:
		// Handle binary expressions (e.g., a == b, a && b)
		left := e.exprToSQL(v.X)
		right := e.exprToSQL(v.Y)
	...
	case *ast.SelectorExpr:
		// Handle struct field access (e.g., c.Country)
		return toSnakeCase(v.Sel.Name)
