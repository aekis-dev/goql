...
// asEntity boxes a reflect.Value as an Entity, unwrapping an interface element and
// taking the address of a plain struct value when needed.
func asEntity(v reflect.Value) (models.Entity, bool) {
	if v.Kind() == reflect.Interface {
		v = v.Elem()
	}
	if v.IsValid() && v.CanInterface() {
		if entity, ok := v.Interface().(models.Entity); ok {
			return entity, true
		}
	}
	if v.Kind() == reflect.Struct {
		...
		ptr.Elem().Set(v)
		if entity, ok := ptr.Interface().(models.Entity); ok {
			return entity, true
		}
	}
	return nil, false
...
// exec helper for Engine
func (ctx *Engine) exec(sqlText string, args ...any) (sql.Result, error) {
	args, err := query.ResolveParams(args, ctx.params)
	if err != nil {
		return nil, err
	}
	if ctx.debugMode {
	...
	if ctx.tx != nil {
		return ctx.tx.ExecContext(ctx.ctx, sqlText, args...)
	}
	return ctx.db.ExecContext(ctx.ctx, sqlText, args...)
}

// query helper for Engine. Params placeholders are substituted here, which is the one
// place every statement passes through.
func (ctx *Engine) query(sqlText string, args ...any) (*sql.Rows, error) {
	args, err := query.ResolveParams(args, ctx.params)
	if err != nil {
		return nil, err
	}
	...
	if ctx.tx != nil {
		return ctx.tx.QueryContext(ctx.ctx, sqlText, args...)
	}
	return ctx.db.QueryContext(ctx.ctx, sqlText, args...)
}
...
func mapColumnsToEntity(entity any, columns []string, values []interface{}) error {
	...
	for i, column := range columns {
		if field, exists := schema.FieldsByDB[column]; exists {
			// A many2one column holds a key, not the related row. Leaving the field nil
			// keeps "not loaded" distinguishable from "loaded but empty"; Preload fills it.
