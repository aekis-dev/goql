...
type Field struct {
	...
	// Preload loads this relation on every read of the owning model. A query may
	// override the set of relations to load, in which case its list replaces these
	// defaults entirely.
...
// LogicalType returns the field's logical column type, inferring one from the Go type
// when the model did not declare it. many2one columns hold a foreign key.
func (fs *Field) LogicalType() ColumnType {
	if fs.Type != "" {
		return fs.Type
	}
